...
public class LanguagePool {
    ...
    public static synchronized ITranslator getTranslator(String xmlFile, String moduleName) {
        existingTranslators.computeIfAbsent(moduleName, k -> new HashMap<>());
    ...
    public static void clearCache(String xmlFile, String moduleName) {
        if (existingTranslators.get(moduleName) != null) {
            existingTranslators.get(moduleName).remove(xmlFile);
        }
    }

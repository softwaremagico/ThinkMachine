...
public class SkillsTable extends BaseElement {
	...
	private static Paragraph createSkillSufix(CharacterPlayer characterPlayer, AvailableSkill availableSkill,
		...
		if (availableSkill.getSpecialization() != null
			...
			if (availableSkill.getSkillDefinition().isLimitedToFaction()) {
				paragraph.add(new Paragraph(availableSkill.getName(), new Font(FadingSunsTheme.getLineItalicFont(),
						fontSize)));
			} else {
				paragraph
						.add(new Paragraph(availableSkill.getName(), new Font(FadingSunsTheme.getLineFont(), fontSize)));
			}
		}
		// Put number at the end.
		if (availableSkill.getSkillDefinition().isNatural()) {
			paragraph
					.add(new Paragraph(DEFAULT_NATURAL_SKILL_VALUE, new Font(FadingSunsTheme.getLineFont(), fontSize)));
		}
	...
	private static Paragraph createSkillSufix(SkillDefinition skillDefinition, int fontSize, int maxColumnWidth) {
		...
		if (skillDefinition.isSpecializable()) {
			...
			if (skillDefinition.isLimitedToFaction()) {
				paragraph.add(new Paragraph(skillDefinition.getName(), new Font(FadingSunsTheme.getLineItalicFont(),
						fontSize)));
			} else {
	...
	private static Paragraph createSpecializedSkill(SkillDefinition skillDefinition, BaseFont font, int fontSize,
		...
		paragraph.add(new Paragraph(CellUtils.getSubStringFitsIn(DEFAULT_WHITE_SPACES, font, fontSize, maxColumnWidth
				- usedWidth), new Font(font, fontSize)));
		paragraph.add(new Paragraph("]", new Font(font, fontSize)));
		return paragraph;
	}

...
public class WeaponsTableLong extends VerticalTable {
	...
	public WeaponsTableLong(CharacterPlayer characterPlayer) {
		...
		for (final Weapon weapon : characterPlayer.getAllWeapons()) {
			...
			if (!weapon.getDamageWithoutArea().endsWith(getTranslator().getTranslatedText("diceAbbreviature"))) {
			...
			addCell(createElementLine(weapon.getShots() == null ? characterPlayer.getStrengthDamangeModification() + ""
					: weapon.getStrengthOrRange(), RANGE_COLUMN_WIDTH, FadingSunsTheme.WEAPONS_SMALL_CONTENT_FONT_SIZE));

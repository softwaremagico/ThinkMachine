...
public class SheetAlternatedBackgroundEvent extends PdfPageEventHelper {
	...
	public void onOpenDocument(PdfWriter writer, Document document) {
		...
		try {
			leftCorner = Image.getInstance(
					SheetAlternatedBackgroundEvent.class.getResource("/" + FadingSunsTheme.LEFT_CORNER_IMAGE));
		...
		try {
			mainTitleRight = Image.getInstance(
					SheetAlternatedBackgroundEvent.class.getResource("/" + FadingSunsTheme.MAIN_TITLE_IMAGE));
			// leftCorner.setAbsolutePosition(IMAGE_BORDER,
			// document.getPageSize().getHeight() - IMAGE_HEIGHT -
			// IMAGE_BORDER);
		...
		try {
			mainTitleLeft = Image.getInstance(
					SheetAlternatedBackgroundEvent.class.getResource("/" + FadingSunsTheme.MAIN_TITLE_IMAGE));
			// leftCorner.setAbsolutePosition(IMAGE_BORDER,
			// document.getPageSize().getHeight() - IMAGE_HEIGHT -
			// IMAGE_BORDER);
